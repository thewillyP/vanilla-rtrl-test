...
def assign_time_points_to_3_stages(loss, d_top, performance_criterion,
                                   topological_criterion, loss_window=1,
                                   topological_window=10, filter_traces=True,
                                   post_process=True, time_point_trigger=4):

    """Simpler stage assignment for just 3 stages"""
    ...
    stage_assignments[np.where(np.logical_and(np.invert(constant_topology),
                                              np.invert(good_performance)))] = 2
    stage_assignments[np.where(good_performance)] = 3

    stage_assignments = stage_assignments.astype(int)

    if post_process:
        sa, tst = post_process_3_stage_assignments(stage_assignments,
                                                   time_point_trigger=time_point_trigger)
...
def post_process_3_stage_assignments(stage_assignments, time_point_trigger=4):
    """For a given set of stage assignments, performs post processing by making
    each period contiguous.
    ...
    if t_stage_transition_prev is not None:
        ret[t_stage_transition_prev:] = 3

    return ret, t_stage_transitions

def assign_time_points_to_3_stages_control(loss, high_loss, low_loss,
                                           loss_window=40, filter_traces=True,
                                           post_process=True, time_point_trigger=4):
    """Control stage assignment for just 3 stages based on loss only"""

    if filter_traces:
        convolved_loss, _ = filter_loss_and_dtop(loss, loss, loss_window=loss_window, topological_window=loss_window)
    else:
        convolved_loss = loss

    ### --- Apply criteria to filtered traces --- ###

    good_performance = convolved_loss[:-1] < low_loss
    okay_performance = np.logical_and(convolved_loss[:-1] >= low_loss,
                                      convolved_loss[:-1] < high_loss)
    bad_performance = convolved_loss[:-1] >= high_loss

    ### --- Assign time points to stages --- ###

    stage_assignments = np.zeros_like(loss[:-1])

    stage_assignments[np.where(bad_performance)] = 1
    stage_assignments[np.where(okay_performance)] = 2
    stage_assignments[np.where(good_performance)] = 3
